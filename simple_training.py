...
Expected usage:
    simple_training.py <model_name> <batch_nr> <nr_epochs> [optional param]
Optional parameters:
...
Example:
    simple_training.py ... > log/$(date \'+%Y%m%d\').test.batch1.log 2> log/$(date \'+%Y%m%d\')test.batch1.err
...
# define model
from src.network.simple_nn import SimpleCNN
cnn_model = SimpleCNN(
